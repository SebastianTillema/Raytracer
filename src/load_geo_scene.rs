...
use crate::scene::{Scene, Color, Sphere, Element, Plane, Triangle};
...
pub fn load_file(file_path: String) -> std::io::Result<GeoData> { 
    let mut file = File::open(file_path)?;  // ? to propagate errors
...
pub fn convert_polymesh_to_trianglemesh(geo_data: &GeoData) -> Vec<usize> {    
    /*
    let mut max_vertex_index = 0;
    let mut k = 0;
    for i in 0.. num_face {
        for j in 0.. face_index_array[i] {
            if vertex_index_array[k + j] > max_vertex_index {
                max_vertex_index += vertex_index_array[k + j];
            }
        }
        k += face_index_array[i];
    } 
    */
    let mut k = 0;
    let mut triangle_index_array: Vec<usize> = Vec::new();

    for i in 0.. geo_data.num_face { // for each face
        for j in 0.. geo_data.face_index_array[i] - 2 {  // for each triangle in the face
...
pub fn create_triangles(vertex_array: Vec<Point>, triangle_index_array: Vec<usize>) -> Vec<Element> {
    ...
    for i in 0.. triangle_index_array.len() / 3 {
        let triangle: Triangle = Triangle {
            point1: vertex_array.get(*triangle_index_array.get(3 * i).unwrap()).unwrap().clone(),
            point2: vertex_array.get(*triangle_index_array.get(3 * i + 1).unwrap()).unwrap().clone(),
            point3: vertex_array.get(*triangle_index_array.get(3 * i + 2).unwrap()).unwrap().clone(),
        };
...
pub fn create_scene_from_file() -> std::io::Result<Scene> {
    // load file
    let file_content = load_file(String::from("geo_test.geo"));
    ...
    // get triangle data
    let triangle_index_array: Vec<usize> = convert_polymesh_to_trianglemesh(&geo_data);
